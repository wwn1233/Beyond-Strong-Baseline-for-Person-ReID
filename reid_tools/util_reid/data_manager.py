...
from util_reid.utils import mkdir_if_missing, write_json, read_json
...
class Market1501_Partial(object):
    ...
    def __init__(self, root='data', num_distra = 2, **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'bounding_box_train')
        self.query_dir = osp.join(self.dataset_dir, 'query')
        self.gallery_dir = osp.join(self.dataset_dir, 'bounding_box_test')
        self.gallery_dir_distractor = osp.join(self.dataset_dir, 'bounding_box_distractor')

        self._check_before_run()

        train, num_train_pids, num_train_imgs = self._process_dir(self.train_dir, relabel=True)
        query, num_query_pids, num_query_imgs = self._process_dir(self.query_dir, relabel=False)
        gallery, num_gallery_pids, num_gallery_imgs = self._process_dir_distrac(self.gallery_dir, self.gallery_dir_distractor, num_distra)
        num_total_pids = num_train_pids + num_query_pids
        num_total_imgs = num_train_imgs + num_query_imgs + num_gallery_imgs
    ...
    def _process_dir_distrac(self, dir_path, dir_path_distrac, num_distrac):
        img_paths = glob.glob(osp.join(dir_path, '*.jpg'))
        pattern = re.compile(r'([-\d]+)_c(\d)')
        ...
        ## distractors
        img_paths = glob.glob(osp.join(dir_path_distrac, '*.jpg'))

        for img_path in img_paths[0:num_distrac*100000]:
            pid, _ = map(int, pattern.search(img_path).groups())
            if pid == -1: continue  # junk images are just ignored
            pid_container.add(pid)
...
class DukeCombine(object):
    ...
    # cameras: 8
    """
    dataset_dir = 'DukeCombine'

    def __init__(self, root='data', WEIGHT_TEST= False,  **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'bounding_box_train')

        if WEIGHT_TEST:
            self.query_dir_2 = osp.join(root,'DukeMTMC4ReID', 'query')
            self.gallery_dir_2 = osp.join(root,'DukeMTMC4ReID', 'bounding_box_test')

            self.query_dir_1 = osp.join(root,'dukemtmc-reid', 'DukeMTMC-reID/query')
            self.gallery_dir_1 = osp.join(root,'dukemtmc-reid', 'DukeMTMC-reID/bounding_box_test')

            self._check_before_run_2()

            train, num_train_pids, num_train_imgs = self._process_dir(self.train_dir, relabel=True)
            query_1, num_query_pids_1, num_query_imgs_1 = self._process_dir(self.query_dir_1, relabel=False)
            gallery_1, num_gallery_pids_1, num_gallery_imgs_1 = self._process_dir(self.gallery_dir_1, relabel=False)

            query_2, num_query_pids_2, num_query_imgs_2 = self._process_dir(self.query_dir_2, relabel=False)
            gallery_2, num_gallery_pids_2, num_gallery_imgs_2 = self._process_dir(self.gallery_dir_2, relabel=False)

            num_total_pids = num_train_pids + num_query_pids_1 + num_query_pids_2
            num_total_imgs = num_train_imgs + num_query_imgs_1 + num_query_imgs_2 + num_gallery_imgs_1 + num_gallery_imgs_2

            print("=> DukeCombine-reID loaded")
            print("Dataset statistics:")
            print("  ------------------------------")
            print("  subset   | # ids | # images")
            print("  ------------------------------")
            print("  train    | {:5d} | {:8d}".format(num_train_pids, num_train_imgs))
            print("  query    | {:5d} | {:8d}".format(num_query_pids_1 + num_query_pids_2, num_query_imgs_1 + num_query_imgs_2))
            print("  gallery  | {:5d} | {:8d}".format(num_gallery_pids_1 + num_gallery_pids_2, num_gallery_imgs_1 + num_gallery_imgs_2))
            print("  ------------------------------")
            print("  total    | {:5d} | {:8d}".format(num_total_pids, num_total_imgs))
            print("  ------------------------------")
            ...
            self.gallery_2 = gallery_2

            self.num_train_pids = num_train_pids
            self.num_query_pids_1 = num_query_pids_1
            self.num_gallery_pids_1 = num_gallery_pids_1
            self.num_query_pids_2 = num_query_pids_2
            self.num_gallery_pids_2 = num_gallery_pids_2
    ...
    def _check_before_run_2(self):
        """Check if all files are available before going deeper"""
        if not osp.exists(self.dataset_dir):
            raise RuntimeError("'{}' is not available".format(self.dataset_dir))
        if not osp.exists(self.train_dir):
            raise RuntimeError("'{}' is not available".format(self.train_dir))
        if not osp.exists(self.query_dir_1):
            raise RuntimeError("'{}' is not available".format(self.query_dir_1))
        if not osp.exists(self.gallery_dir_1):
            raise RuntimeError("'{}' is not available".format(self.gallery_dir_1))
        if not osp.exists(self.query_dir_2):
            raise RuntimeError("'{}' is not available".format(self.query_dir_2))
        if not osp.exists(self.gallery_dir_2):
            raise RuntimeError("'{}' is not available".format(self.gallery_dir_2))
...
__img_factory = {
    ...
    'msmt17': MSMT17,
    'dukecombine': DukeCombine,
    'dukemtmc4reid': DukeMTMC4ReID,
}
