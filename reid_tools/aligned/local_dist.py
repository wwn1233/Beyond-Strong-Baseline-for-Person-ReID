...
def smooth_ln_distance(distance,beta = 0.75):
    smoothLn_sign = (distance <= beta).detach().float()
    distance_smoothLn = smoothLn_sign * (-torch.log(torch.abs(1 - distance)))+ \
                        (1 - smoothLn_sign) * ((distance - beta)/(1 - beta) - torch.log(1 - beta- torch.Tensor([0])[0].cuda()))
    distance_smoothLn[torch.isnan(distance_smoothLn)] = 0
    return distance_smoothLn
def batch_local_dist(x, y):
  ...
  #dist_mat = (torch.exp(dist_mat) - torch.exp(-dist_mat)) / (torch.exp(dist_mat) + torch.exp(-dist_mat))
  #dist_mat = (torch.exp(dist_mat/2) - 1.) / (torch.exp(dist_mat/2) + 1.)
  
  # shape [N]
  dist = shortest_dist(dist_mat.permute(1, 2, 0))
  return dist

def batch_local_dist_smoothln(x, y):
  """
  Args:
    x: pytorch Variable, with shape [N, m, d]
    y: pytorch Variable, with shape [N, n, d]
  ...
  # shape [N, m, n]
  dist_mat = batch_euclidean_dist(x, y)
  dist_mat = smooth_ln_distance(dist_mat,beta = 0.7)
  print('Max:{},Min:{}'.format(torch.max(dist_mat),torch.min(dist_mat)))
  dist_mat = (torch.exp(dist_mat) - 1.) / (torch.exp(dist_mat) + 1.)
  #dist_mat = (torch.exp(dist_mat) - torch.exp(-dist_mat)) / (torch.exp(dist_mat) + torch.exp(-dist_mat))
  #dist_mat = (torch.exp(dist_mat/2) - 1.) / (torch.exp(dist_mat/2) + 1.)
