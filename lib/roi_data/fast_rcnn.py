...
def add_fast_rcnn_blobs(blobs, im_scales, roidb):
    ...
    for im_i, entry in enumerate(roidb):
        if cfg.TRAIN.BALANCE_SAMPLE:
            frcn_blobs = _sample_rois_balance_sample(entry, im_scales[im_i], im_i)
        else:
...
def _sample_rois_balance_sample(roidb, im_scale, batch_idx):
    ...
    for ii in range(len(gt_num_sort)):
        dispatch = gt_pair[1, gt_num_sort[ii]] if gt_pair[1, gt_num_sort[ii]] <= average_label_num else average_label_num
        roi_max_num[gt_pair[0, gt_num_sort[ii]]] = dispatch
        fg_remain -= dispatch
