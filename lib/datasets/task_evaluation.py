...
def evaluate_all_slowtest(
    dataset, all_boxes, all_segms, all_keyps, use_matlab=False
):
    """Evaluate "all" tasks, where "all" includes box detection, instance
    segmentation, and keypoint detection.
    """
    all_results = evaluate_boxes_slowtest(
        dataset, all_boxes, use_matlab=use_matlab
    )
    logger.info('Evaluating bounding boxes is done!')
    if cfg.MODEL.MASK_ON:
        results = evaluate_masks_slowtest(dataset, all_boxes, all_segms)
        all_results[dataset.name].update(results[dataset.name])
        logger.info('Evaluating segmentations is done!')
    # if cfg.MODEL.KEYPOINTS_ON:
    #     results = evaluate_keypoints(dataset, all_boxes, all_keyps, output_dir)
...
def evaluate_boxes_fasttest(dataset, all_boxes, use_matlab=False):
    """Evaluate bounding box detection."""
    logger.info('Evaluating detections')
    not_comp = not cfg.TEST.COMPETITION_MODE
    if _use_json_dataset_evaluator(dataset):
        detection_AP = json_dataset_evaluator.evaluate_boxes_fasttest(
            dataset, all_boxes, use_salt=not_comp, cleanup=not_comp
        )
        # box_results = _coco_eval_to_box_results(coco_eval)
    elif _use_cityscapes_evaluator(dataset):
        logger.warn('Cityscapes bbox evaluated using COCO metrics/conversions')
...
def evaluate_boxes_slowtest(dataset, all_boxes, use_matlab=False):
    """Evaluate bounding box detection."""
    logger.info('Evaluating detections')
    not_comp = not cfg.TEST.COMPETITION_MODE
    if _use_json_dataset_evaluator(dataset):
        coco_eval = json_dataset_evaluator.evaluate_boxes_slowtest(
            dataset, all_boxes, use_salt=not_comp, cleanup=not_comp
        )
        box_results = _coco_eval_to_box_results(coco_eval)
    elif _use_cityscapes_evaluator(dataset):
...
def evaluate_masks_slowtest(dataset, all_boxes, all_segms):
    """Evaluate instance segmentation."""
    logger.info('Evaluating segmentations')
    not_comp = not cfg.TEST.COMPETITION_MODE
    if _use_json_dataset_evaluator(dataset):
        coco_eval = json_dataset_evaluator.evaluate_masks_slowtest(
            dataset,
            all_boxes,
            all_segms,
            use_salt=not_comp,
...
def _use_json_dataset_evaluator(dataset):
    ...
    return dataset.name.find('coco_') > -1 or dataset.name.find('drive') > -1  or cfg.TEST.FORCE_JSON_DATASET_EVAL
