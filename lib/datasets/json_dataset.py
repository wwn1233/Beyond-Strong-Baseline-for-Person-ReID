...
class JsonDataset(object):
    ...
    def __init__(self, name, Is_debuglearn=False):
        ...
        if Is_debuglearn is True:
            self.COCO = COCO(DATASETS[name][ANN_FN])

            id_list=[]
            #extract 'images'
            image_list= self.COCO.dataset['images']
            for ii, this_dict in enumerate(image_list):
                this_id = this_dict['id']
                # generate random id index
                # num_image= len(self.COCO.dataset['images'])
            ...
            for ii, this_dict in enumerate(image_list):
                ...
                if this_id in id_list:
                    image_list_new.append(this_dict)
            self.COCO.dataset['images'] = image_list_new

            # extract 'annotation'
            annotation_list = self.COCO.dataset['annotations']
            annotation_list_new = []
            for jj,this_dict in enumerate(annotation_list):
                this_id = this_dict['image_id']
                if this_id in id_list:
                    annotation_list_new.append(this_dict)
            self.COCO.dataset['annotations'] = annotation_list_new
    ...
    def get_roidb(
        ...
        if cfg.DEBUG:
            ...
            roidb = copy.deepcopy(self.COCO.loadImgs(image_ids))#[440:1800]
        for entry in roidb:
            self._prep_roidb_entry(entry)
        if gt:
            # Include ground-truth object annotations
            cache_filepath = os.path.join(self.cache_path, self.name+'_gt_roidb.pkl')
            if False and os.path.exists(cache_filepath) and not cfg.DEBUG:
    ...
    def _add_gt_annotations(self, entry):
        ...
        for ix, obj in enumerate(valid_objs):
            # if obj['category_id']==3 or obj['category_id']==4 or obj['category_id']==5 or obj['category_id']==7 or obj['category_id']==8 or obj['category_id']==9:
    ...
    def _init_keypoints(self):
        ...
        elif 'car' in self.category_to_id_map:  # added by Weinong Wang *********************************
