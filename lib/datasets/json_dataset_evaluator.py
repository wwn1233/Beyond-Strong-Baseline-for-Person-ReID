...
def evaluate_masks(
    ...
    if json_dataset.name.find('test') == -1 or json_dataset.name.find('vali') == -1:
...
def evaluate_masks_slowtest(
    ...
    # Only do evaluation on non-test sets (annotations are undisclosed on test)
    if json_dataset.name.find('test') == -1 or json_dataset.name.find('vali') == -1:
        coco_eval = _do_segmentation_eval_slowtest(json_dataset, results)
    else:
        coco_eval = None
    # # Optionally cleanup results json file
    # if cleanup:
...
def _do_segmentation_eval(json_dataset, res_file, output_dir):
    ...
    segm_AP_each = _log_detection_eval_metrics(json_dataset, coco_eval)
    pandas.DataFrame(segm_AP_each).to_csv('segm_AP_each_group.csv')
    eval_file = os.path.join(output_dir, 'segmentation_results.pkl')
    save_object(coco_eval, eval_file)
    logger.info('Wrote json eval results to: {}'.format(eval_file))
    return coco_eval

def _do_segmentation_eval_slowtest(json_dataset, res_file):
...
def evaluate_boxes_fasttest(
    json_dataset, all_boxes, use_salt=True, cleanup=False
):
    # res_file = os.path.join(
    #     output_dir, 'bbox_' + json_dataset.name + '_results'
    # )
    ...
    logger.info(
        'generating bbox results')

    # Only do evaluation on non-test sets (annotations are undisclosed on test)
    # if json_dataset.name.find('test') == -1:
    detection_AP = _do_detection_eval_fasttest(json_dataset, results)
    # else:
    #     coco_eval = None
    # # Optionally cleanup results json file
    # if cleanup:
...
def evaluate_boxes_slowtest(
    ...
    # Only do evaluation on non-test sets (annotations are undisclosed on test)
    if json_dataset.name.find('test') == -1 or json_dataset.name.find('vali') == -1:
        coco_eval = _do_detection_eval_slowtest(json_dataset, results)
    else:
        coco_eval = None
...
def _do_detection_eval_fasttest(json_dataset, res_file):
    coco_dt = json_dataset.COCO.loadRes(res_file)
    coco_eval = COCOeval(json_dataset.COCO, coco_dt, 'bbox')
    coco_eval.evaluate()
    coco_eval.accumulate()
    detection_AP = _log_detection_eval_metrics_fasttest(json_dataset, coco_eval)
    # pandas.DataFrame(detection_AP_each).to_csv('detection_AP_each_group.csv')
    # eval_file = os.path.join(output_dir, 'detection_results.pkl')
    # save_object(coco_eval, eval_file)
    # logger.info('Wrote json eval results to: {}'.format(eval_file))
...
def _do_detection_eval_slowtest(json_dataset, res_file):
    coco_dt = json_dataset.COCO.loadRes(res_file)
    coco_eval = COCOeval(json_dataset.COCO, coco_dt, 'bbox')
    coco_eval.evaluate()
    coco_eval.accumulate()
...
def _log_detection_eval_metrics_fasttest(json_dataset, coco_eval):
...
def _log_detection_eval_metrics(json_dataset, coco_eval):
    ...
    for cls_ind, cls in enumerate(json_dataset.classes):
        ...
        ap_each = np.mean(precision, 1)
        ap_each_group[:, cls_ind - 1] = ap_each
    # np.savetxt('AP_each_group.csv',ap_each_group)
    # pandas.DataFrame(ap_each_group).to_csv('AP_each_group.csv')
    logger.info('~~~~ Summary metrics ~~~~')
    coco_eval.summarize()

    return ap_each_group
...
def _coco_kp_results_one_category(json_dataset, boxes, kps, cat_id):
    ...
    for i, image_id in enumerate(image_ids):
        if len(boxes[i]) == 0:
            continue
        kps_dets = kps[i]
        scores = boxes[i][:, 4].astype(np.float)
        if len(kps_dets) == 0:
            continue
