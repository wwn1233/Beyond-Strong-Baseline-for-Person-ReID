...
def load_ckpt(model, ckpt):
    ...
    for name in ckpt:
        try:
            if mapping[name]:
                if name == 'Box_Outs.cls_score.weight' and ckpt[name].shape[0] == 81:
                    state_dict[name] = model_state_dict[name]
                    state_dict[name][2,:] = ckpt[name][1,:]  #person
                    state_dict[name][4, :] = ckpt[name][10, :] # traffic light
                elif name == 'Box_Outs.cls_score.bias' and ckpt[name].shape[0] == 81:
                    state_dict[name] = model_state_dict[name]
                    state_dict[name][2] = ckpt[name][1]  # person
                    state_dict[name][4] = ckpt[name][10]  # traffic light
                elif name == 'Box_Outs.bbox_pred.weight' and ckpt[name].shape[0] == 324:
                    state_dict[name] = model_state_dict[name]
                    state_dict[name][8:12,:] = ckpt[name][4:8,:]  #person
                    state_dict[name][16:20, :] = ckpt[name][40:44, :] # traffic light
                elif name == 'Box_Outs.bbox_pred.bias' and ckpt[name].shape[0] == 324:
                    state_dict[name] = model_state_dict[name]
                    state_dict[name][8:12] = ckpt[name][4:8]  #person
                    state_dict[name][16:20] = ckpt[name][40:44] # traffic light
                elif name == 'Mask_Outs.classify.weight' and ckpt[name].shape[0] == 81:
                    state_dict[name] = model_state_dict[name]
                    state_dict[name][2,:,:,:] = ckpt[name][1,:,:,:]  # person
                    state_dict[name][4,:,:,:] = ckpt[name][10,:,:,:]  # traffic light
                elif name == 'Mask_Outs.classify.bias' and ckpt[name].shape[0] == 81:
                    state_dict[name] = model_state_dict[name]
                    state_dict[name][2] = ckpt[name][1]  # person
                    state_dict[name][4] = ckpt[name][10]  # traffic light

                else:
                    # if cfg.MODEL.KEYPOINTS_ON and name.split('.')[0] == 'Box_Outs':  #added by weinong wang, for keypoints load ckpt
                    #     print('ignore parameters: {}'.format(name))
                    #     continue
                    if cfg.MODEL.KEYPOINTS_ON and not cfg.KRCNN.TONPY:
                        if name == 'Box_Outs.cls_score.weight' and ckpt[name].shape[0] >= 6:
                            state_dict[name] = model_state_dict[name]
                            state_dict[name][1, :] = ckpt[name][3, :]  # car
                            state_dict[name][0, :] = ckpt[name][0, :]  #
                        elif name == 'Box_Outs.cls_score.bias' and ckpt[name].shape[0] >= 6:
                            state_dict[name] = model_state_dict[name]
                            state_dict[name][1] = ckpt[name][3]  # car
                            state_dict[name][0] = ckpt[name][0]  #
                        elif name == 'Box_Outs.bbox_pred.weight' and ckpt[name].shape[0] >= 24:
                            state_dict[name] = model_state_dict[name]
                            state_dict[name][4:8, :] = ckpt[name][12:16, :]  # car
                            state_dict[name][0:4, :] = ckpt[name][0:4, :]  #
                        elif name == 'Box_Outs.bbox_pred.bias' and ckpt[name].shape[0] >= 24:
                            state_dict[name] = model_state_dict[name]
                            state_dict[name][4:8] = ckpt[name][12:16]  # car
                            state_dict[name][0:4] = ckpt[name][0:4]  #
                        else:
            ...
            if cfg.MODEL.KEYPOINTS_ON:
                ...
                ValueError('please check mapping[name]!:{}'.format(name))
    model.load_state_dict(state_dict, strict=False)

#added by weinong wang, for keypoint
def load_ckpt_keypoint(model, ckpt):
    """Load checkpoint"""
    mapping, _ = model.detectron_weight_mapping
    model_state_dict = model.state_dict()
    state_dict = {}
    for name in ckpt:
        if cfg.MODEL.KEYPOINTS_ON and name.split('.')[0].split('_')[0] == 'Keypoint':
            state_dict[name] = ckpt[name]
        #if name == 'Box_Outs.cls_score.weight':
        #    state_dict[name] = ckpt[name]
        #if name == 'Box_Outs.cls_score.bias':
        #    state_dict[name] = ckpt[name]                   
        #if name == 'Box_Outs.bbox_pred.weight':
        #    state_dict[name] = ckpt[name]                    
        #if name == 'Box_Outs.bbox_pred.bias':
        #    state_dict[name] = ckpt[name]
    model.load_state_dict(state_dict, strict=False)
