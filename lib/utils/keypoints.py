...
def get_keypoints():
    ...
    keypoints = ['lft', 'rft', 'rfb', 'lfb', 'ldt', 'rdt', 'rdb', 'ldb']
    keypoint_flip_map = {
        'lft': 'rft',
        'lfb': 'rfb',
        'ldt': 'rdt',
...
#
#         # mid = np.transpose(maps[i], [2, 0, 1])
#         # roi_map_probs = np.transpose(scores_to_probs(mid),[1, 2, 0])
#         # roi_map = cv2.resize(
#         #     roi_map_probs, (roi_map_width, roi_map_height))#,
#         # roi_map = np.transpose(roi_map, [2, 0, 1])
#         # roi_map_probs = roi_map
#
#         w = roi_map.shape[2]
...
def heatmaps_to_keypoints(maps, rois):
    ...
    for i in range(len(rois)):
        ...
        for k in range(cfg.KRCNN.NUM_KEYPOINTS):
            pos = roi_map[k, :, :].argmax()
            x_int_fea = pos % w
            y_int_fea = (pos - x_int_fea) // w
            assert (roi_map[k, y_int_fea, x_int_fea] ==
                    roi_map[k, :, :].max())

            x_int = x_int_fea*roi_map_width/56
            y_int = y_int_fea * roi_map_height / 56
            ...
            xy_preds[i, 0, k] = x + offset_x[i]
            xy_preds[i, 1, k] = y + offset_y[i]
            xy_preds[i, 2, k] = roi_map[k, int(y_int_fea), int(x_int_fea)]
            xy_preds[i, 3, k] = roi_map[k, int(y_int_fea), int(x_int_fea)]
...
def compute_oks(src_keypoints, src_roi, dst_keypoints, dst_roi):
    ...
    sigmas = sigmas[0:8]  #  added by  weinong wang
    vars = (sigmas * 2)**2
