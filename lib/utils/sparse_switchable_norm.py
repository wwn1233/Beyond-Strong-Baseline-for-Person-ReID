...
class SSN2d(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.997, using_moving_average=True, last_gamma=False):
        ...
        self.mean_weight = nn.Parameter(torch.ones(3), requires_grad=False)
        self.var_weight = nn.Parameter(torch.ones(3), requires_grad=False)
        self.register_buffer('running_mean', torch.zeros(1, num_features, 1))
        self.register_buffer('running_var', torch.zeros(1, num_features, 1))

        self.rad = 0.
        self.register_buffer('mean_fixed', torch.LongTensor([1]))
        self.register_buffer('var_fixed', torch.LongTensor([1]))
