...
import modeling.ResNet_reid as ResNet
...
# FPN with ResNet
# ---------------------------------------------------------------------------- #
def fpn_ResNet18_conv5_body_prune(cfg_prune): ## added by Weinong Wang #***********************************************************3
    return fpn(
        ResNet.ResNet18_conv5_body_prune,
        fpn_level_info_ResNet18_conv5(),
        cfg_prune = cfg_prune
        )
...
class fpn(nn.Module):
    ...
    """
    def __init__(self, conv_body_func, fpn_level_info, P2only=False, cfg_prune = None): ## added by Weinong Wang #***********************************************************3
        super().__init__()
        ...
        for i in range(self.num_backbone_stages):
            if cfg.FPN.USE_GN:
                ...
                if cfg.REID.FPN:
                # added BN and relu by Weinognwang. 20181204
                    self.posthoc_modules.append(nn.Sequential(
        ...
        # conv_body has its own _init_weights function ## modified by Weinong Wang
        if cfg_prune is None:  # ***********************************************************3
            self.conv_body = conv_body_func()  # e.g resnet
        else:
            self.conv_body = conv_body_func(cfg_prune)
            self.conv_top = nn.Conv2d(cfg_prune[-1], fpn_dim, 1, 1, 0)  ## yes,  update the self.conv_top
    ...
    def forward(self, x):
        if cfg.RESNETS.DROPBLOCK:
            self.conv_body.dropblock.step()  # increment number of iterations
        ...
        for i in range(1, self.conv_body.convX):
            ...
            if cfg.RESNETS.DROPBLOCK:
                if i == 3 or i == 4:
                    # print('*********WWN')
                    conv_body_blobs[-1] = self.conv_body.dropblock(conv_body_blobs[-1])

        #
        if cfg.REID.REGULARIZED_POOLING:
            # print('Not implemented!')
            return conv_body_blobs[-2], conv_body_blobs[-1]
        else:
            return conv_body_blobs[-1]
...
def fpn_level_info_ResNet34_conv5():
    return FpnLevelInfo(
        blobs=('res5_2_sum', 'res4_5_sum', 'res3_3_sum', 'res2_2_sum'),
        dims=(512, 256, 128, 64),
        spatial_scales=(1. / 32., 1. / 16., 1. / 8., 1. / 4.)
