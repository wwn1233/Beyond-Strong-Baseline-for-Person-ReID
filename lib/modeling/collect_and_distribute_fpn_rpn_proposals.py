...
class CollectAndDistributeFpnRpnProposalsOp(nn.Module):
    ...
    def forward(self, inputs, roidb, im_info):
        ...
        """
        if cfg.DISTILLER.L1RankedStructureParameterPruner:
            inputs = [blob_this.data.numpy() for blob_this in inputs]
        ...
        if self.training:
            ...
            # not matter).
            if cfg.KRCNN.TRAIN_EXPAND_ROI > -1:
                distance = (rois[:,3] - rois[:,1])
                rois[:,1] = rois[:,1] - cfg.KRCNN.TRAIN_EXPAND_ROI * distance/2
                rois[:,3] = rois[:,3] + cfg.KRCNN.TRAIN_EXPAND_ROI * distance/2
                distance = (rois[:,4] - rois[:,2])
                rois[:,2] = rois[:,2] - cfg.KRCNN.TRAIN_EXPAND_ROI * distance/2
                rois[:,4] = rois[:,4] + cfg.KRCNN.TRAIN_EXPAND_ROI * distance/2
